...
def findSepSet(graph, A, B, SCCs, ancestors, acyclic):
    ...
    for n in range(len(other_variables)+1):
        ...
        for possible_sep_set in possible_sep_sets:
            if isSeparated(graph, A, B, possible_sep_set, ancestors, acyclic=acyclic, SCCs=SCCs) == 'blocked':
...
# can use tarjans algorithm to make it more efficient
def findPosAncestors(graph):
    """
    Returns ancestors of all vertices in graph.
        :param graph: CausalGraph
    """

    # select the matrix with directed edges and the matrix with (partial) directed edges
    len_vars = len(graph.all_variables)
    directed_matrix = np.zeros((len_vars, len_vars))
    partial_directed_matrix = np.zeros((len_vars, len_vars))
    for i in range(len_vars):
        for j in range(len_vars):
            if graph.all_variables[j] in graph.adjacencies(graph.all_variables[i]):
                if graph.isDirectedEdge(graph.all_variables[i], graph.all_variables[j]):
                    directed_matrix[i,j] = 1
                    partial_directed_matrix[i,j] = 1
                elif graph.incomingArrowType(graph.all_variables[j], graph.all_variables[i]) != graph.tail and graph.incomingArrowType(graph.all_variables[i], graph.all_variables[j]) != graph.head:
                    partial_directed_matrix[i,j] = 1

    # use taylor series trick of e^A to quickly find which nodes are ancestors
    # we only have to take taylor series of length of number of variables
    # because that will include the possible paths
    # we can ignore the factorial because we are only interested if the outcome is non-zero
    anc = np.zeros((len_vars,len_vars))
    pos_anc = np.zeros((len_vars,len_vars))
    DMM = directed_matrix
    PDMM = partial_directed_matrix
    for i in range(1,len_vars):
        anc += DMM
        pos_anc += PDMM
        DMM = DMM @ directed_matrix
        PDMM = PDMM @ partial_directed_matrix
    anc += DMM
    pos_anc += PDMM
    ...
    for j in range(len_vars):
        ancestors[graph.all_variables[j]] = [graph.all_variables[i] for i in np.nonzero(anc[:,j])[0]] + [graph.all_variables[j]]
        pos_ancestors[graph.all_variables[j]] = [graph.all_variables[i] for i in np.nonzero(pos_anc[:,j])[0] if anc[i,j]==0]

    return ancestors, pos_ancestors
...
def findStartDiscriminatingPath(graph, A, B, C):
    ...
    for variable in graph.adjacencies(C):
        ...
        if graph.score(C, variable)!=-1 and graph.isDirectedEdge(variable, C):
        ...
        while paths != []:
            ...
            for path in paths:
                for variable in parents:
                    ...
                    # check if variable is possible next vertex on the path and real edge
                    if variable in graph.adjacencies(path[-1]) and variable not in path and graph.score(path[-1], variable)!=-1:
    ...
    while paths != []:
        ...
        for path in paths:
            for variable in graph.adjacencies(path[-1]):
                if variable!=B and variable not in path:
                    # check if the triple is unshielded
                    if path[-2] not in graph.adjacencies(variable):
                        # check if there is an o-o edge connecting the variables and is real
                        if graph.incomingArrowType(variable, path[-1]) == graph.unknown and graph.incomingArrowType(path[-1], variable) == graph.unknown and graph.score(path[-1], variable)!=-1:
                            new_path = path.copy()
                            new_path.append(variable)
                            if variable == D:
            ...
            for variable in graph.adjacencies(path[-1]):
                if variable not in path:
                    ...
                    if len(path) == 1 or path[-2] not in graph.adjacencies(variable):
                        # check for potentially directed edge and real edge
                        if graph.incomingArrowType(variable, path[-1]) != graph.tail and graph.incomingArrowType(path[-1], variable) != graph.head and graph.score(path[-1], variable)!=-1:
                            new_path = path.copy()
                            new_path.append(variable)
                            if variable == B:
                                final_paths.append(new_path)
